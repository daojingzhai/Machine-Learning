...
# -*- coding: utf-8 -*-
"""
Created on Sat May  5 11:10:13 2018
...
def preprocess():
    with open('Groceries.csv', 'r', encoding='utf-8') as csv_file:
        ...
        for line in csv_reader:
            yield line[1].strip('{').strip('}').split(',')

def print_patterns():
    sum = 0
    print('Patterns:======================================================\n')
    for j in patterns:
       print(j,patterns.get(j)/9835)
       sum = sum + 1
    print('======================================================\n')
    return sum
...
if __name__ == '__main__':
    ts = time.time()
    dataset = list(preprocess())
    minsup = 0.1
    minconf = 0.04
    patterns = find_frequent_patterns(dataset, 9835*minsup)
    rules = generate_association_rules(patterns, minconf)
    te = time.time()
    process = psutil.Process(os.getpid())
    ##print rules and patterns
    sum_itemset = print_patterns()
    sum_rules = print_rules()
    ##print costs
    print("Memory cost: ",process.memory_info().rss/1024/1024,"MB")
    print("Time cost: ",te-ts,"s")
    print("FP-growth returned",sum_itemset,sum_rules)